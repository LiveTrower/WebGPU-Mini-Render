...
import shader from "./shaders/my_shader.wgsl";
...
export class Renderer {
    ...
    async createAssets() {
        ...
        await this.triangleMaterial.initialize(this.device, "lion", this.materialGroupLayout);
        await this.quadMaterial.initialize(this.device, "Planks", this.materialGroupLayout);
    ...
    prepareScene(renderables: RenderData, camera: Camera) {
        ...
        mat4.perspective(projectionMatrix, Math.PI/4, 800/600, 0.1, 10);

        const viewMatrix = renderables.view_transform;
        ...
        this.device.queue.writeBuffer(this.uniformBuffer, 0, new Float32Array(viewMatrix)); 
