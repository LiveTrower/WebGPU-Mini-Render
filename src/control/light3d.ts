...
export class Light3D {
	...
	constructor(device: GPUDevice) {
		...
		this.lightReverseProjectionMatrix = mat4.multiply(
			depthRangeRemapMatrix,
			this.lightProjectionMatrix,
		);
		this.lightViewProjectionMatrix = mat4.multiply(
			this.lightReverseProjectionMatrix,
		...
		this.energySlider = document.getElementById(
			"light-energy",
		) as HTMLInputElement;
		...
		this.shadowCheckbox = document.getElementById(
			"shadow-checkbox",
		) as HTMLInputElement;
	...
	private setupPositionListeners(): void {
		const updateFromInputs = () => {
			const x = this.posXInput
				? parseFloat(this.posXInput.value)
				: this.position[0];
			const y = this.posYInput
				? parseFloat(this.posYInput.value)
				: this.position[1];
			const z = this.posZInput
				? parseFloat(this.posZInput.value)
				: this.position[2];

			this.position = vec3.create(
				Number.isNaN(x) ? this.position[0] : x,
				Number.isNaN(y) ? this.position[1] : y,
				Number.isNaN(z) ? this.position[2] : z,
			);
