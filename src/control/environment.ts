...
export class Environment {
	...
	constructor() {
		...
		this.fogCheckbox = document.getElementById(
			"fog-checkbox",
		) as HTMLInputElement;
		this.colorPicker = document.getElementById("fog-color") as HTMLInputElement;
		this.fogDensitySlider = document.getElementById(
			"fog-density",
		) as HTMLInputElement;
		this.fogDensityValue = document.getElementById("fog-density-value");
		this.fogSkyEffectSlider = document.getElementById(
			"fog-sky-effect",
		) as HTMLInputElement;
		this.fogSkyEffectValue = document.getElementById("fog-sky-effect-value");
		this.depthFogCheckbox = document.getElementById(
			"depth-fog-checkbox",
		) as HTMLInputElement;
		this.fogNearSlider = document.getElementById(
			"fog-near",
		) as HTMLInputElement;
	...
	private updateFogDepthUIVisibility(): void {
		const nearContainer = this.fogNearSlider?.closest(
			".property",
		) as HTMLElement | null;
		const farContainer = this.fogFarSlider?.closest(
			".property",
		) as HTMLElement | null;
